...
def run_pipeline(url, steps):
    ...
    if "scrape" in steps or "all" in steps:
        ...
        if not raw_json_path:
            ...
            return
        print(f"Scraped data saved to: {raw_json_path}")
    else:
        # Try to find existing file if skipping scrape
        raw_json_path = f"data/raw/{video_id}.json"
        if not Path(raw_json_path).exists():
             print(f"Raw data not found at {raw_json_path}. Cannot proceed.")
             return
    ...
    if "preprocess" in steps or "all" in steps:
        ...
        if not Path(cleaned_csv_path).exists():
             print(f"Cleaned data not found at {cleaned_csv_path}. Cannot proceed.")
             return
    ...
    if "embeddings" in steps or "all" in steps:
        ...
        if not Path(emb_path).exists():
             print(f"Embeddings not found at {emb_path}. Cannot proceed.")
             return
    ...
    if "topics" in steps or "all" in steps:
        ...
        if not Path(topics_path).exists():
             print(f"Topics data not found at {topics_path}. Cannot proceed.")
             return None
